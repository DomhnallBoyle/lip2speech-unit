...
python create_dataset.py $type $root_dir generate_file_list
...
python create_dataset.py $type $root_dir manifests
...
./synthesise.sh $root_dir/label $synthesis_dir $checkpoint_path

# setup vocoder
echo Setting up vocoder...
python create_dataset.py $type $root_dir vocoder $synthesis_dir

# run vocoder
echo Running vocoder...
./vocoder.sh $synthesis_dir/vocoder $vocoder_dir