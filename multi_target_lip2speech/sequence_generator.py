...
class MultiTargetSequenceGenerator(SequenceGenerator):
    ...
    def _generate(
        ...
        for i, target_length in enumerate(sample['target_lengths']):
            ...
            if True in (sample['target'][i]==self.eos):
                for trg_i in range(len(sample['target'][i])):
        ...
        if encoder_outs[0]['encoder_out_mel'] is not None:
            mels = encoder_outs[0]['encoder_out_mel'].cpu().numpy()
        ...
        trg_lens = sample['target_lengths'].index_select(0, new_order)
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if reorder_state is not None:
                ...
                trg_lens = trg_lens.index_select(0, reorder_state)

            if step >= max_len:
                lprobs = torch.zeros_like(encoder_outs[0]['encoder_out'][0])
            else:
                lprobs = self.model.single_model.get_normalized_probs({'encoder_out': encoder_outs[0]['encoder_out'][step].div_(self.temperature)}, log_probs=True)
            ...
            ###
            lprobs[:, self.tgt_dict.bos()] = -math.inf  # never select bos
            lprobs[:, self.eos] = -math.inf  # never select eos
            ...
            if step >= max_len:
                ...
                ii = trg_lens <= step
                if True in ii:
                    lprobs[ii, :self.eos] = -math.inf
                    lprobs[ii, self.eos + 1 :] = -math.inf
                    lprobs[ii, self.eos] = 0
            ...
            cand_scores, cand_indices, cand_beams = self.search.step(
                ...
                tokens[:, : step + 1],  ### no use
                original_batch_idxs,  ### no use
            )
