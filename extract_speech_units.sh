...
. $virtual_env && cd $working_dir && python $rel_script_path --feature_type hubert --kmeans_model_path $kmeans_model_path --acoustic_model_path $acoustic_model_path --layer $layer --manifest_path $root/${type}_unit_manifest.txt --out_quantized_file_path=$root/label/${type}.unt --extension ".wav" --hide-fname
