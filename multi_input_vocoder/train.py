...
from dataset_multi_input import MelCodeDataset, mel_spectrogram, get_dataset_filelist
...
def train(rank, local_rank, a, h):
    ...
    if rank == 0:
        validset = MelCodeDataset(validation_filelist, h.segment_size, h.code_hop_size, h.mel_hop_size, h.n_fft, h.num_mels, h.hop_size,
                               h.win_size, h.sampling_rate, h.fmin, h.fmax, False, n_cache_reuse=0, fmax_loss=h.fmax_for_loss,
                               device=device, multispkr=h.get('multispkr', None), code_dict_path=h.code_dict_path)
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            if train_loader.dataset.use_noise:
                x['mel'] = x['mel'] + torch.randn_like(x['mel']) * train_loader.dataset.noise_factor
            y_g_hat = generator(**x)
